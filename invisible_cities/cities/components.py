...
from .. io     .hits_io           import          hits_from_df_new
...
def hits_and_kdst_from_files(paths: List[str]) -> Iterator[Dict[str,Union[HitCollection, pd.DataFrame, MCInfo, int, float]]]:
    ...
    for path in paths:
        ...
        with tb.open_file(path, "r") as h5in:
            ...
            for evtinfo in event_info:
                event_number, timestamp = evtinfo.fetch_all_fields()
                hits = hits_from_df_new(hits_df.loc[hits_df.event == event_number])
...
def dhits_and_kdst_from_files(paths: List[str]) -> Iterator[Dict[str,Union[HitCollection, pd.DataFrame, MCInfo, int, float]]]:
    """Reader of the files, yields HitsCollection, pandas DataFrame with
    kdst info, run_number, event_number and timestamp."""
    for path in paths:
        try:
        ...
        with tb.open_file(path, "r") as h5in:
            ...
            for evtinfo in event_info:
                event_number, timestamp = evtinfo.fetch_all_fields()
                dhits = hits_from_df_new(dhits_df.loc[dhits_df.event == event_number])
...
def copy_generalE_to_Ep_hit_attribute_(hitc : HitCollection) -> HitCollection:
    """
    The functions copies values of Ec (or E if there is no Ec) attributes into Ep attributes.
    Takes as input HitCollection and returns a copy.
    """
    mod_hits = []
...
### move definition of writers??? in other cases (e.g. pmaps_writer) , they are in pmap_io
